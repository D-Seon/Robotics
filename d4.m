...
% ★ 관절 2 (\theta_2) 구동 범위 설정: 반 바퀴(pi)가 아닌 한 바퀴(2*pi) 전체를 회전하도록 변경
num_steps = 240;
...
xlim(ax2, [0, num_steps]); ylim(ax2, [-10, 370]); % 360도 분석을 위해 y축 범위를 370까지 확장
...
curve_theta2 = plot(ax2, NaN, NaN, '-', 'Color', [0.0, 1.0, 0.5], 'LineWidth', 2.5, 'DisplayName', '\theta_2 (Active Full Circle)');
