% Synchronized Initial State: Fixed Theta2 (0 deg) with Rotating Theta1 (MATLAB R2026a)
% Starting Point: x = 2.0, y = 0 (theta1 = 0, theta2 = 0)
clear; clc; close all;
...
% ★ 초기 위치 x=2, y=0 자세를 만족하기 위해 \theta_2를 0도로 완벽 고정합니다.
theta2_fixed_deg = 0; 
theta2_fixed = deg2rad(theta2_fixed_deg);

% 관절 1 (\theta_1)의 회전 궤적 생성 (초기 자세인 0도부터 시작하여 360도 전체 회전)
num_steps = 240;
theta1_range = linspace(0, 2*pi, num_steps);

%% 2. 시각화 그래픽스 환경 구축 (듀얼 가로형 레이아웃)
fig = figure('Name', 'Synchronized \theta_2 Control & Full-Circle \theta_1 Analysis Dashboard', ...
             'NumberTitle', 'off', ...
             'Color', [0.08, 0.09, 0.11], ...
             'Position', [100, 200, 1250, 550]); 

%% 3. [좌측 스크린 - 물리 공간 궤적 시뮬레이션]
...
title(ax1, sprintf('SCREEN 01: Physical Workspace (Fixed \\theta_2 = %d^\\circ)', theta2_fixed_deg), ...
      'Color', [0.9, 0.95, 1.0], 'FontName', 'Segoe UI', 'FontWeight', 'bold');

% 핸드 종단(링크 2 끝)이 그릴 실시간 원형 궤적선 (네온 시안 컬러)
...
%% 4. [우측 스크린 - 관절 공간 변위 실시간 그래프]
ax2 = subplot(1, 2, 2, 'Parent', fig, ...
              'Color', [0.12, 0.14, 0.18], 'XColor', [0.4, 0.45, 0.55], 'YColor', [0.4, 0.45, 0.55], ...
              'GridColor', [0.25, 0.28, 0.35], 'GridAlpha', 0.4);
grid(ax2, 'on'); hold(ax2, 'on');
xlim(ax2, [0, num_steps]); ylim(ax2, [-10, 370]);
...
title(ax2, 'SCREEN 02: Real-Time Joint Trajectory Curves', 'Color', [0.9, 0.95, 1.0], 'FontName', 'Segoe UI', 'FontWeight', 'bold');

% 실시간 궤적 플롯 객체 선언 (Line 클래스 속성오류 방지를 위해 'DisplayName' 문법 고수)
curve_theta1 = plot(ax2, NaN, NaN, '-', 'Color', [0.0, 1.0, 0.5], 'LineWidth', 2.5, 'DisplayName', '\theta_1 (Active Rotating)');
curve_theta2 = plot(ax2, NaN, NaN, '--', 'Color', [1.0, 0.4, 0.0], 'LineWidth', 2.0, 'DisplayName', '\theta_2 (Locked 0^o)');
...
%% 8. 정기구학(Forward Kinematics) 기반 실시간 루프 연산
x_history = []; y_history = [];
...
for i = 1:num_steps
    ...
    theta2 = theta2_fixed; % 0도 완벽 고정
    
    % 차트용 데이터 누적 히스토리
    time_axis = [time_axis, i];
    t1_history = [t1_history, rad2deg(theta1)];
    t2_history = [t2_history, theta2_fixed_deg];
    ...
    %% HUD 대시보드 스펙 정보 갱신
    hud_string = sprintf(['  [SYNC MODE HUD]\n  ------------------\n  Start At: (2.0, 0)\n  Locked \\theta_2:   %2d^o\n  Active \\theta_1: %5.1f^o\n\n  [CURRENT HAND]\n  X Coord : %5.2f m\n  Y Coord : %5.2f m'], ...
                           theta2_fixed_deg, rad2deg(theta1), x2, y2);
